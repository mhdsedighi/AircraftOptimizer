function cost=cost_func(input,N_condition,N_design,N_morph,N_trim,N_act,all_state,geo,all_struc,body,act,engine,ref,finilize)
...
if finilize
    for k=1:N_condition
        [aero,all_perf(k),all_trim_cost(k),all_act(k,:),all_state2(k),all_geo(k),all_struc2(k),all_engine(k),all_ref(k)] = compute_aircraft(design_code,morph_code(k,:),trim_code(k,:),actmass_code,all_state(k),geo,all_struc(k),body,act,engine,ref);
        aero.sonicCP=0;
        aero.sonicWarning=0;
    ...
    parfor k=1:N_condition
        ...
        [~,all_perf(k),all_trim_cost(k),all_act(k,:),~,~,~,all_engine(k),~] = compute_aircraft(design_code,morph_code(k,:),trim_code(k,:),actmass_code,all_state(k),geo,all_struc(k),body,act,engine,ref);
        %     x(j)=all_act;
    end
end
