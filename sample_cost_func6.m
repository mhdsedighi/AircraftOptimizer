function [cost_dual] = sample_cost_func6(input)
global doplot
...
state.ALT=       0.3048*50000;         %Altitude, m
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
struc.rho(:,:)=structure_rho*ones(geo.nwing);
...
%
[ vertex_info ] = get_vertex_info(geo,lattice);
wingno=1;
partno=1;
Raxle=[input(2) input(3) input(4)];
Raxle=Raxle/norm(Raxle);
hinge_pos=[0 0 0];
alpha=input(1)*pi/180;
isattached=1;

[lattice]=part_rotation(wingno,partno,geo,lattice,vertex_info,Raxle,hinge_pos,alpha,isattached);
%
...
Cj=0.6;
Wbegin=Wto;
Wend=Wbegin-1800*2.20462;
Endurance=1/Cj*(CL/CD)*log(Wbegin/Wend);
Range=V*CL/CD/Cj*log(Wbegin/Wend)*1609.34;
