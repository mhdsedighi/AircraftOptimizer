...
geo.startx=      [0 0];        %X coordinate of each wing apex   
...
geo.nx=        [10
                10];%Number of panels in the X direction of ];
    
 geo.ny=        [5          %Number of panels in the Y direction of          %each partiton
                 5];
...
geo.foil(:,:,1)= [{'0012'}     %Airfoil file inboard profile
                  {'0'}];
              
geo.foil(:,:,2)= [{'0'}     %Airfoil file inboard profile
                  {'0'}];
...
% [lattice1]=wingrotation2(wingno,geo,lattice1,Raxle,hinge_pos,alpha);
