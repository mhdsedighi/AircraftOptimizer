function cost=cost_func3(input,N_condition,N_design,N_morph,N_trim,N_act,all_state,geo,all_struc,body,act,engine,ref,finilize)
...
if finilize
    for k=1:N_condition
        [aero,all_perf(k),all_trim_cost(k),all_act(k,:),all_state2(k),all_geo(k),all_struc2(k),all_engine(k),all_ref(k)] = compute_aircraft3(design_code,morph_code(k,:),trim_code(k,:),actmass_code,all_state(k),geo,all_struc(k),body,act,engine,ref);
        aero.sonicCP=0;
        aero.sonicWarning=0;
...
cost_perf2=1/(all_perf(5).ROC+1);
if cost_perf2<=0
    cost_perf2=-10*cost_perf2;
end
cost_perf3=1/(all_state(4).AS+1);

% cost=cost_trim^2*(cost_st+1)*(cost_perf2)*(cost_perf3)*(cost_perf1+1)+mass_error;
cost=cost_trim*(cost_st+1)*(cost_perf2+1)*(cost_perf3+1)*(cost_perf1+1)+mass_error;
