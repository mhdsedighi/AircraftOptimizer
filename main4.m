...
all_struc(1).m_fuel=W_f*0.1;
all_struc(2).m_fuel=W_f*0.1;
...
i_Zwing=[-1 1];
i_bfrac1=[0.01 0.7];
i_sweep_angle=[0 15]*pi/180;
i_twist_angle=[-20 20]*pi/180;
% i_taper=[0.1 1]*pi/180;


% i_course_portion=[0 0.5];
i_rot_angle=[-5 5]*pi/180;
i_Raxle_local=[0 0 0;1 1 1];

i_alpha=[-1 15]*pi/180;
i_elevator_angle=[-30 30]*pi/180;
i_throttle=[0 1];
V_service=357*0.44704;
i_AS=[0.4 1]*V_service;
...
fun=@(input)cost_func5(input,N_condition,N_design,N_morph,N_trim,N_act,all_state,geo,all_struc,body,act,engine,ref,finilize);
...
options = saoptimset(options,'TolFun', 1e-10);
options = saoptimset(options,'MaxFunEvals', 1e9);
options = saoptimset(options,'MaxIter', 1e9);
% options = saoptimset(options,'StallIterLimit', StallIterLimit_Data);
options = saoptimset(options,'InitialTemperature', 10000);
...
% x=input;
finilize=1;
cost=cost_func5(x,N_condition,N_design,N_morph,N_trim,N_act,all_state,geo,all_struc,body,act,engine,ref,finilize);
