function [results]=solver9_actuator(results,state,geo,lattice,act,ref)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
rhs=(setboundary5(lattice,state,geo,ref))';
%disp('rhs... ok')
...
c3=lattice.COLLOC-ones(size(lattice.COLLOC,1),1)*act.pos;
...
for j=1:nofderiv
    ...
    Fprim(:,j,:)=state.rho*cross(Wind,G);			    %Force per unit length
        ...
        %%%%%added by mehdi
        F(:,j,1)=act.panel_consider.*F(:,j,1);
        F(:,j,2)=act.panel_consider.*F(:,j,2);
        F(:,j,3)=act.panel_consider.*F(:,j,3);
        %%%%%
