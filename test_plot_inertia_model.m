function test_plot_inertia_model(geo,lattice)
N=100;
mat=lattice.XYZ;
% geo=struc.geo;
% [n,~]=size(mat);
...
for i=1:1
    ...
    for j=1:1
        for cc=1:10
            ...
            k2=k1+geo.nx(i,j)-1;
            k2_f=k2+geo.fnx(i,j);
            %       k3=k1+(geo.nx(i,j)+geo.fnx(i,j))*(1);
            k3=k2_f+1;
            k4=k3+geo.nx(i,j)-1;
            k4_f=k4+geo.fnx(i,j);
            pause(0.5)
            ...
            pause(0.5)
            plot3(mat(k1,2,1),mat(k1,2,2),mat(k1,2,3),'r*')
            pause(0.5)
            plot3(mat(k2,3,1),mat(k2,3,2),mat(k2,3,3),'r*')
            ...
            %                 plot3(mat(k1,2,1),mat(k1,2,2),mat(k1,2,3),'r*')
            %                 pause(0.5)
            %                 plot3(mat(k1,3,1),mat(k1,3,2),mat(k1,3,3),'r*')
            %                 pause(0.5)
            %                 plot3(mat(k1,4,1),mat(k1,4,2),mat(k1,4,3),'r*')
            %                 pause(0.5)
            %
            ...
            k1=k1+(geo.nx(i,j)+geo.fnx(i,j));
            
%             if geo.symetric(i)==1
%                 k1=k1+(geo.nx(i,j)+geo.fnx(i,j))*1;
%                 pause(0.5)
%                 plot3(mat(k1,2,1),mat(k1,2,2),mat(k1,2,3),'r*')
%                 pause(0.5)
%                 plot3(mat(k2,3,1),mat(k2,3,2),mat(k2,3,3),'r*')
