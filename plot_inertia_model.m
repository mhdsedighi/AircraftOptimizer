function plot_inertia_model(struc)
N=3;
geo=struc.geo;
mat=struc.XYZ;
[n,~]=size(mat);
...
for i=1:nwing
    ...
    for j=1:npart
        ...
        if geo.fsym(i,j)==1
            [xyz_r,xyz_flap_r]=place_foil(Tr,X1(l,:),geo.foil(i,j,1),c_r,0,geo.fc(i,j),geo.flap_vector(i,j),N);
            [xyz_t,xyz_flap_t]=place_foil(Tt,X3(l,:),geo.foil(i,j,2),c_t,0,geo.fc(i,j),geo.flap_vector(i,j),N);
            ...
            [xyz_r,xyz_flap_r1]=place_foil(Tr,X1(l,:),geo.foil(i,j,1),c_r,0,geo.fc(i,j),geo.flap_vector(i,j),N);
            [~,xyz_flap_r2]=place_foil(Tr,X1(l,:),geo.foil(i,j,1),c_r,0,geo.fc(i,j),-geo.flap_vector(i,j),N);
            [xyz_t,xyz_flap_t1]=place_foil(Tt,X3(l,:),geo.foil(i,j,2),c_t,0,geo.fc(i,j),geo.flap_vector(i,j),N);
            [~,xyz_flap_t2]=place_foil(Tt,X3(l,:),geo.foil(i,j,2),c_t,0,geo.fc(i,j),-geo.flap_vector(i,j),N);
...
trimesh(hullFacets,DT.Points(:,1),DT.Points(:,2),DT.Points(:,3),'FaceColor',facecolor,'EdgeColor',edgecolor)
